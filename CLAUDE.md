# Authorization — Plugin Context for Claude

> Plugin for the **AlfacodeTeam PhpServicePlatform** (Sentinel) kernel.
> Package `alfacode-team/hkm-plugin-authorization` · namespace `Plugins\Authorization\` · solves `authorization.policy`
>
...
| | |
|---|---|
| `solves` | `authorization.policy` |
| `requires` | `database.management` |
| `exposes` | `AuthorizationServiceContract` |
| `emits` | — |
...
`config[]` is also what `hkm plugins enable authorization` seeds into the project's
...
|---|---|---|---|
| `AUTHZ_MODEL_PATH` | string | no | — |
| `AUTHZ_POLICY_TABLE` | string | no | — |
...
The Casbin policy engine wrapped for GDA. Policy storage goes through
`DatabasePort` via `DatabasePolicyAdapter` (table `casbin_rule`, overridable with
`AUTHZ_POLICY_TABLE`).

The `Enforcer` is an **internal binding** — only `AuthorizationServiceContract`
is exposed. Nothing outside this plugin touches Casbin types.
...
Model config: `config/rbac_model.conf` (override with `AUTHZ_MODEL_PATH`). Run
the bundled migration to create `casbin_rule`.

## WHERE AUTHORIZATION DECISIONS BELONG

Authorization is a **Service-layer** concern. A `SecurityLayer` establishes *who
you are*; this plugin answers *may they*. Calling it from a security layer
applies the rule to every route in the application.

```
✗ Importing an Enforcer or any Casbin class outside this plugin
✗ Building a policy string from unvalidated user input
✗ Caching an allows() result across requests without keying on tenant — a cached
  "yes" from one tenant answering for another is the worst kind of bug here
```
